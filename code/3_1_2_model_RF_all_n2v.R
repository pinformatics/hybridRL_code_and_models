...
model %>% 
  write_rds("result/model_RF_2_all_n2v.rds")
...
(df_review <- 
    df_collection %>% 
    mutate(
      pred_probs_va = map2(model_rf_full, df_va, ~predict(.x, .y, na.action = na.pass, type = "prob")$match),
      pred_probs_ts = map2(model_rf_full, df_ts, ~predict(.x, .y, na.action = na.pass, type = "prob")$match),
      preds_va = map2(model_rf_full, df_va, ~predict(.x, .y, na.action = na.pass)),
      preds_ts = map2(model_rf_full, df_ts, ~predict(.x, .y, na.action = na.pass)),
...
#df_review_sub %>% 
#  write_csv("result/model_RF_2_all_n2v_review.csv")
